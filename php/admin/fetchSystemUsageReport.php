...
// Ensures the client’s JavaScript can parse the system usage statistics as JSON, maintaining compatibility with web standards for the dashboard.
...
// Ensures all date and time operations, like filtering bookings or logging errors, use the correct time zone for consistency in the Kenyan context.
...
// Establishes a connection to the database to query system usage statistics for the admin dashboard.
...
// Variable: Stores the start date for filtering bookings.
...
// Captures the start date from the client’s request to filter booking statistics, ensuring safe database queries.
$startDate = isset($_GET['start']) ? $conn->real_escape_string($_GET['start']) : null;

// Variable: Stores the end date for filtering bookings.
...
// Captures the end date from the client’s request to filter booking statistics, ensuring secure query execution.
...
// Holds the condition to filter booking records by BookingDate within the specified date range for targeted statistics.
$bookingDateFilter = "";
...
// String: Concatenates " WHERE DATE(BookingDate) BETWEEN '$startDate' AND '$endDate'" to filter bookings by BookingDate, where DATE() extracts the date part of a datetime field.
// Applies the date range filter to limit booking data to the client-specified period, ensuring relevant system usage reports.
    $bookingDateFilter = " WHERE DATE(BookingDate) BETWEEN '$startDate' AND '$endDate'";
}

// Variable: SQL query string for system usage statistics.
// String: Defines a query with subqueries to select COUNT(*) as TotalBookings and COUNT(DISTINCT CustomerID) as ActiveCustomers from 'bookingdetails' with $bookingDateFilter, COUNT(*) as ActiveStaff from 'staff', and COUNT(*) as ActiveBuses from 'bus'.
// Aggregates key metrics (total bookings, unique customers, staff, and buses) to display system activity on the admin dashboard.
$sql = "
    SELECT 
        (SELECT COUNT(*) FROM bookingdetails $bookingDateFilter) AS TotalBookings,
        (SELECT COUNT(DISTINCT CustomerID) FROM bookingdetails $bookingDateFilter) AS ActiveCustomers,
        (SELECT COUNT(*) FROM staff) AS ActiveStaff,
        (SELECT COUNT(*) FROM bus) AS ActiveBuses
";

// Variable: Stores the result of the statistics query.
// Object: $conn->query($sql) is a MySQLi method that executes the SQL query and returns a result object containing the statistics or false if the query fails.
// Holds the retrieved counts (bookings, customers, staff, buses) for processing into the JSON response.
$result = $conn->query($sql);

// Variable: Initializes the response data structure for statistics.
// Array: Creates an associative array with 'response_status' set to 'success', 'applied_filters' as a subarray containing $startDate and $endDate, and 'debug_query' set to $sql for debugging.
// Organizes the JSON response with metadata (status, filters, query) to accompany the statistics data for dashboard display.
$responseData = [
    'response_status' => 'success',
    'applied_filters' => ['startDate' => $startDate, 'endDate' => $endDate],
    'debug_query' => $sql // Debug query
];

// Conditional statement: Handles query execution outcomes.
// Boolean and integer checks: Tests if $result is false (query failure) or if $result->num_rows, a MySQLi property, is greater than 0 (rows returned); also handles the case of no rows.
// Processes statistics if available, sends an error for query failures, or indicates no data if the result is empty.
if ($result === false) {
    // Function call: Discards buffered output to ensure a clean response.
// String: ob_end_clean() is a PHP built-in function that clears all data in the output buffer, discarding any unintended output.
// Prevents stray text from corrupting the JSON error message sent to the client.
    ob_end_clean();
    // Output statement: Sends a JSON error response to the client.
// String: echo outputs text; json_encode() converts an array ['response_status' => 'error', 'error' => $conn->error] to a JSON string, where $conn->error is a MySQLi property with the query error message.
// Informs the client of a query failure, providing error details for debugging on the dashboard.
    echo json_encode(['response_status' => 'error', 'error' => $conn->error]);
    // Function call: Terminates script execution immediately.
// String: exit() is a PHP built-in function that stops the script from running further.
// Halts processing to prevent further actions after a query error.
    exit;
} elseif ($result->num_rows > 0) {
    // Variable: Stores the query result row.
// Array: $result->fetch_assoc() is a MySQLi method that retrieves a single row as an associative array with keys TotalBookings, ActiveCustomers, ActiveStaff, and ActiveBuses.
// Extracts the statistics data for inclusion in the JSON response.
    $row = $result->fetch_assoc();
    // Array operation: Combines statistics with response data.
// Array: array_merge() is a PHP built-in function that merges $responseData and $row, adding TotalBookings, ActiveCustomers, ActiveStaff, and ActiveBuses to $responseData.
// Integrates the statistics into the response array for client-side dashboard display.
    $responseData = array_merge($responseData, $row);
} else {
    // Array operation: Updates response with error for empty results.
// Strings: Sets $responseData['response_status'] to 'error' and $responseData['error'] to 'No data found'.
// Notifies the client that no statistics were retrieved, ensuring the dashboard handles empty data gracefully.
...
// String: ob_end_clean() is a PHP built-in function that clears all data in the output buffer; echo outputs text; json_encode() converts $responseData to a JSON string containing statistics and metadata.
// Sends system usage statistics (bookings, customers, staff, buses) and metadata (status, filters, query) to the client for dashboard display.
ob_end_clean();
...
// Returns system usage statistics and metadata to the client.
