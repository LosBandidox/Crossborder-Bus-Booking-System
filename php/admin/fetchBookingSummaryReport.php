...
// Sets the response format to JSON, ensuring client-side JavaScript can process the booking data correctly.
...
// Loads the database connection settings to fetch booking details from the International Bus Booking System’s database.
...
// Captures the start date for filtering bookings, ensuring safe database input.
...
// Captures the end date for filtering bookings, ensuring safe database input.
...
// Holds the SQL condition to filter bookings by BookingDate within a date range.
$bookingDateFilter = "";

// Conditional statement: Logic to build date filter clause.
// Boolean check: Tests if both $startDate and $endDate are non-null.
// Adds a WHERE clause to filter bookings by date range if both dates are provided.
if ($startDate && $endDate) {
    // String assignment: Constructs a WHERE clause for date filtering.
// String: Concatenates " WHERE DATE(BookingDate) BETWEEN '$startDate' AND '$endDate'" to filter BookingDate within the specified range.
// Applies the date range filter to the SQL query for targeted booking retrieval.
    $bookingDateFilter = " WHERE DATE(BookingDate) BETWEEN '$startDate' AND '$endDate'";
}

// Variable: SQL SELECT query string.
// String: Defines a query to select BookingID, CustomerID, ScheduleID, BookingDate, and TravelDate from the 'bookingdetails' table, appending $bookingDateFilter and sorting by BookingDate in descending order (DESC).
// Retrieves booking records, optionally filtered by date, for display or reporting.
$sql = "
    SELECT 
        BookingID, CustomerID, ScheduleID, BookingDate, TravelDate
    FROM 
        bookingdetails
    $bookingDateFilter
    ORDER BY 
...
// Stores the retrieved booking records for processing.
$result = $conn->query($sql);

// Variable: Booking data storage.
// Array: Initializes an empty array to store booking records as associative arrays.
// Collects booking details for inclusion in the JSON response.
$bookingData = [];

// Variable: Response data structure.
// Array: Initializes an associative array with 'response_status' set to 'success', 'applied_filters' containing $startDate and $endDate, 'row_count' set to 0, and 'data' referencing $bookingData.
// Organizes the JSON response with metadata (status, filters, count) and booking data.
$responseData = [
...
// Processes each booking record to build the response data.
    while ($row = $result->fetch_assoc()) {
        // Array operation: Appends booking row to data array.
// Array: Adds $row (containing BookingID, CustomerID, ScheduleID, BookingDate, TravelDate) to $bookingData.
// Collects booking details for the JSON response.
        $bookingData[] = $row;
        // Array operation: Increments row count.
// Integer: Increments $responseData['row_count'] by 1 for each row.
// Tracks the number of booking records retrieved for metadata.
...
// Sends the booking data and metadata (status, filters, count) to the client for processing.
