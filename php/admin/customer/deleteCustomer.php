...
// Establishes a connection to the database to perform deletion operations for customer, bookings, and payment records.
...
// Ensures a valid Customer ID is provided before attempting deletions, preventing invalid requests from proceeding.
...
// Captures the Customer ID to identify which customer, bookings, and payment records to delete from the database.
$customerId = $_GET['id'];
...
try {
    ...
    // Guarantees data consistency by ensuring all deletions (payments, bookings, and customer) succeed or fail as a unit, preventing partial deletions.
    $conn->begin_transaction();

    // Variable: SQL query string to delete payment records for the customer.
    // String: Defines a query to DELETE FROM paymentdetails WHERE BookingID IN (SELECT BookingID FROM bookingdetails WHERE CustomerID = ?), using a placeholder (?) for secure parameter binding.
    // Specifies the deletion of payment records linked to the customer through their bookings, ensuring related data is removed.
    $paySql = "DELETE FROM paymentdetails WHERE BookingID IN 
               (SELECT BookingID FROM bookingdetails WHERE CustomerID = ?)";
    
    // Variable: Stores the prepared statement for payment deletion.
    // Object: $conn->prepare($paySql) is a MySQLi method that compiles the SQL query with the placeholder, returning a statement object for binding and execution.
    ...
    // Boolean check: Tests if $payStmt is false, indicating a preparation failure (e.g., syntax error or database issue).
    // Throws an exception to trigger rollback if preparation fails, ensuring no partial changes occur.
    if (!$payStmt) throw new Exception('Failed to prepare payment delete query');
    ...
    // String: execute() is a MySQLi method that runs the prepared statement, deleting payment records linked to the Customer ID.
    ...
    // String: close() is a MySQLi method that releases the prepared statement ($payStmt), freeing memory.
    // Ensures efficient resource management after the payment deletion is complete.
    $payStmt->close();

    // Variable: SQL query string to delete booking records for the customer.
    // String: Defines a query to DELETE FROM bookingdetails WHERE CustomerID = ?, using a placeholder for secure binding.
    // Specifies the deletion of booking records associated with the given Customer ID, ensuring related data is removed.
    $bookSql = "DELETE FROM bookingdetails WHERE CustomerID = ?";
    
    // Variable: Stores the prepared statement for booking deletion.
    // Object: $conn->prepare($bookSql) compiles the SQL query, returning a statement object for binding and execution.
    ...
    // Boolean check: Tests if $bookStmt is false, indicating a preparation failure.
    // Throws an exception to trigger rollback if preparation fails, ensuring data consistency.
    if (!$bookStmt) throw new Exception('Failed to prepare booking delete query');
    
    // Method call: Binds the Customer ID to the booking deletion statement.
    // String: bind_param("i", $customerId) binds $customerId as an integer to the placeholder in the query.
    // Securely links the Customer ID to the booking deletion query, ensuring safe execution.
    $bookStmt->bind_param("i", $customerId);
    
    // Method call: Executes the booking deletion query.
    // String: execute() runs the prepared statement, deleting booking records matching the Customer ID.
    ...
    // String: close() releases the prepared statement ($bookStmt), freeing memory.
    // Ensures efficient resource management after the booking deletion is complete.
    $bookStmt->close();

    // Variable: SQL query string to delete the customer record.
    // String: Defines a query to DELETE FROM customer WHERE CustomerID = ?, using a placeholder for secure binding.
    // Specifies the deletion of the customer record identified by the Customer ID, completing the removal process.
    $sql = "DELETE FROM customer WHERE CustomerID = ?";
    
    // Variable: Stores the prepared statement for customer deletion.
    // Object: $conn->prepare($sql) compiles the SQL query, returning a statement object for binding and execution.
    // Prepares the query to safely delete the customer record, maintaining security against SQL injection.
    $stmt = $conn->prepare($sql);
    
    // Conditional statement: Checks if the customer deletion query preparation was successful.
    // Boolean check: Tests if $stmt is false, indicating a preparation failure.
    // Throws an exception to trigger rollback if preparation fails, ensuring data consistency.
    if (!$stmt) throw new Exception('Failed to prepare customer delete query');
    
    // Method call: Binds the Customer ID to the customer deletion statement.
    // String: bind_param("i", $customerId) binds $customerId as an integer to the placeholder in the query.
    // Securely links the Customer ID to the customer deletion query, ensuring safe execution.
    $stmt->bind_param("i", $customerId);
    
    // Method call: Executes the customer deletion query.
    // String: execute() runs the prepared statement, deleting the customer record matching the Customer ID.
    // Removes the customer record from the customer table as part of the transaction.
    $stmt->execute();
    
    // Method call: Frees the customer statement resources.
    // String: close() releases the prepared statement ($stmt), freeing memory.
    // Ensures efficient resource management after the customer deletion is complete.
    $stmt->close();
    ...
    // Confirms all deletions (payments, bookings, and customer), ensuring data consistency in the database.
    $conn->commit();

    // Output statement: Sends a JSON success response to the client.
    // String: echo outputs text; json_encode() is a PHP built-in function that converts an array ['status' => 'success', 'message' => 'Customer and related data deleted'] to a JSON string, a data format for web communication.
    // Informs the client’s JavaScript that the deletion was successful, allowing the dashboard or interface to update (e.g., remove the customer from the display).
    echo json_encode(['status' => 'success', 'message' => 'Customer and related data deleted']);
} catch (Exception $e) {
    ...
    // Prevents partial deletions (e.g., payments deleted but customer not deleted) if an error occurs, maintaining data integrity.
    $conn->rollback();
    
    // Output statement: Sends a JSON error response to the client.
    // String: echo outputs text; json_encode() is a PHP built-in function that converts an array ['status' => 'error', 'message' => 'Failed to delete customer: ' . $e->getMessage()] to a JSON string, a data format for web communication.
    ...
    echo json_encode(['status' => 'error', 'message' => 'Failed to delete customer: ' . $e->getMessage()]);
}
