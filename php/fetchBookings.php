...
// Starts a session to access the user’s email, enabling authentication checks for secure booking history retrieval in the International Bus Booking System.
session_start();
...
// Loads the database connection settings to query customer and booking data for the booking history.
...
// Ensures only logged-in users can access their booking history, preventing unauthorized access to sensitive data.
if (isset($_SESSION["Email"])) {
    ...
    // Stores the email to fetch the customer’s ID from the database, linking booking history to the correct user.
    ...
    // Retrieves the customer’s unique ID to query their booking details securely.
    ...
    // Proceeds to fetch booking details if a customer is found, or sends an error if not.
    if ($result->num_rows > 0) {
        ...
        // Extracts the customer’s data (CustomerID) for use in booking queries.
        $row = $result->fetch_assoc();

        // Variable: Customer ID storage.
        // String: $row['CustomerID'] is the CustomerID field from the fetched associative array.
        // Stores the customer’s unique ID to query their bookings.
        $customerID = $row['CustomerID'];

        // Variable: SQL SELECT query string.
        // String: Defines a query joining 'bookingdetails', 'scheduleinformation', 'bus', and 'route' tables to select BookingID, SeatNumber, Status, DepartureTime, Cost, BusNumber, StartLocation, and Destination where CustomerID matches a placeholder (?).
        // Retrieves all booking details for the customer to display their booking history.
        $sql = "SELECT b.BookingID, b.SeatNumber, b.Status,
                       s.DepartureTime, s.Cost, 
                       bus.BusNumber, r.StartLocation, r.Destination
        ...
        // Attaches the customer ID to the query safely, preventing SQL injection for secure booking data retrieval.
        $stmt->bind_param("i", $customerID);

        // Method call: Query execution function.
        // String: execute() is a MySQLi method that runs the prepared statement on the database.
        // Executes the query to fetch the customer’s booking details.
        $stmt->execute();

        // Variable: Query result storage.
        // Object: $stmt->get_result() is a MySQLi method that retrieves the result set from the executed prepared statement.
        // Stores the booking data for processing into a response.
        $result = $stmt->get_result();

        // Variable: Bookings storage.
        // Array: An empty array initialized to hold associative arrays of booking records.
        // Prepares to collect booking details (e.g., BookingID, SeatNumber) for the JSON response to the frontend.
        $bookings = [];
        ...
        // Processes each booking record to build the $bookings array for the booking history display.
        while ($row = $result->fetch_assoc()) {
            // Array operation: Append booking record to array.
            // Array: $row is an associative array containing BookingID, SeatNumber, Status, DepartureTime, Cost, BusNumber, StartLocation, and Destination.
            // Adds each booking record to $bookings for the JSON response.
            $bookings[] = $row;
        }

        // Output statement: JSON success response output.
        // String: echo is a PHP statement that outputs the result of json_encode(), a PHP built-in function that converts an associative array with 'status' ('success') and 'bookings' ($bookings) to a JSON string.
        // Sends the customer’s booking history to the client (e.g., JavaScript) for rendering on the booking history page.
    ...
    // Output statement: JSON error response output.
    // String: echo is a PHP statement that outputs the result of json_encode(), a PHP built-in function that converts an associative array with 'status' ('error') and 'message' ('Please log in to view your booking history') to a JSON string.
    // Sends an error to the client if the user is not logged in, prompting them to log in before accessing booking history.
    echo json_encode(["status" => "error", "message" => "Please log in to view your booking history"]);
}
